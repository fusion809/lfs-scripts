...
set lmargin 13
set ylabel "Frequency" rotate by 0 offset -1.5,0

set style fill solid 0.8
# Get data stats
stats "/home/fusion809/lfs-scripts/boots.dat" nooutput
binwidth = (log10(STATS_max)-log10(STATS_min))/STATS_records
set boxwidth binwidth * 0.9
set logscale x
set logscale y

logbin(x,width) = 10**(width*floor(log10(x)/width))

count  = STATS_records
mean   = STATS_mean
median = STATS_median
iqr    = STATS_up_quartile - STATS_lo_quartile

xmin = STATS_min - binwidth
xmax = STATS_max + binwidth

set table $hist
plot "/home/fusion809/lfs-scripts/boots.dat" using (logbin($1,binwidth)):(1.0) smooth freq
unset table
...
if (mean < median) {
    set label 2 sprintf("Mean: %.2fs", mean)   at mean-5*offset,   graph 0.97 right
    set label 3 sprintf("Median: %.2fs", median) at median+17.5*offset, graph 0.97 right
    set label 4 sprintf("IQR: %.2fs", iqr) at (xmax-xmin)*0.2+xmin, graph 0.97 left
} else {
    set label 2 sprintf("Mean: %.2fs", mean)   at mean+5*offset,   graph 0.97 left
    set label 3 sprintf("Median: %.2fs", median) at median-17.5*offset, graph 0.97 left
    set label 4 sprintf("IQR: %.2fs", iqr) at (xmax-xmin)*0.2+xmin, graph 0.97 left
}
